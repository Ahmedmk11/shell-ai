...
async def get_file(owner: str, repo: str, path: str) -> dict:
    ...
    """
    response = await github_client.get(f"/repos/{owner}/{repo}/contents/{path}")
    response.raise_for_status()
    data = response.json()
...
async def create_branch(owner: str, repo: str, branch: str, base_branch: str = "main") -> dict:
    ...
    """
    ref_response = await github_client.get(f"/repos/{owner}/{repo}/git/ref/heads/{base_branch}")
    ref_response.raise_for_status()
    sha = ref_response.json()["object"]["sha"]

    response = await github_client.post(f"/repos/{owner}/{repo}/git/refs", json={
...
async def create_pull_request(owner: str, repo: str, title: str, head: str, base: str) -> dict:
    ...
    """
    response = await github_client.post(f"/repos/{owner}/{repo}/pulls", json={
        "title": title,
        "head": head,
        "base": base
    })
...
async def list_pull_requests(owner: str, repo: str) -> list:
    ...
    """
    response = await github_client.get(f"/repos/{owner}/{repo}/pulls")
    response.raise_for_status()
    return [
        {
            "number": pr["number"],
...
async def create_issue(owner: str, repo: str, title: str, body: str) -> dict:
    ...
    """
    response = await github_client.post(f"/repos/{owner}/{repo}/issues", json={
        "title": title,
        "body": body
    })
    response.raise_for_status()
...
async def create_fork(owner: str, repo: str) -> dict:
    ...
    """
    response = await github_client.post(f"/repos/{owner}/{repo}/forks")
    response.raise_for_status()
    data = response.json()
    return {
        "full_name": data["full_name"],
